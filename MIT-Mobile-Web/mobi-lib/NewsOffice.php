...
//UNC EDIT
        $body = @file_get_contents('http://www.dailytarheel.com/section/pageOne.xml');
  ...
  private function xml_node2item($xml_node) {
      $images = array();
      //UNC EDIT
      foreach($xml_node->getElementsByTagname('enclosure url') as $image_xml) {
	...
	'unixtime' => strtotime(getNodeValue($xml_node,'pubDate')),
      );
